 ...
 */
struct slot * reachDesiredElement(int row, int column, struct slot * initialSlot,  int * pSlotRowNum, int * pSlotColNum,struct slot ** board, char * pSlotType){

	bool found = false;
	//current slot
	struct slot * currentSlot = initialSlot;
	printf("\nYour Current Slot is (%d,%d) %s\n", pSlotRowNum, pSlotColNum, pSlotType);
	printf("\nSearching Board For Your New Slot (%d,%d)\n", row, column);
